...
func ErrorResponse(c *fiber.Ctx, etyp ErrorType, etag ErrorTag, emsg error) error {
	if errorSchema == nil {
		log.Fatalf("restconf: errors schema not loaded")
	}
	var err error
	var eroot, e yangtree.DataNode
	status := fiber.StatusOK
	eroot, err = yangtree.New(errorSchema)
	if err != nil {
	...
	if _, err := eroot.Insert(e, nil); err != nil {
		log.Fatalf("restconf: fault in error report: %v", err)
	}
...
func GetResponse(c *fiber.Ctx, node yangtree.DataNode) error {
	var marshal func(node yangtree.DataNode, prefix, indent string, option ...yangtree.Option) ([]byte, error)
	marshal = yangtree.MarshalXMLIndent
	accepts := c.Accepts("text/json", "text/yaml", "text/xml",
		"application/xml", "application/json", "application/yaml",
	...
	case strings.HasSuffix(accepts, "yaml"):
		...
		marshal = yangtree.MarshalYAMLIndent
	default:
		return ErrorResponse(c.Status(fiber.StatusNotAcceptable),
			ETypeTransport, ETagInvalidValue, errors.New("not supported Content-Type"))
	}
	b, err := marshal(node, "", " ")
	if err != nil {
